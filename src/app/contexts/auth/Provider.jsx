...
export function AuthProvider({ children }) {
    ...
    const initialize = async () => {
      try {
        ...
        if (authToken && isTokenValid(authToken)) {
          ...
          const response = await axios.get("http://vps65389.dreamhostps.com:5000/user/profile");
          dispatch({
            type: "INITIALIZE",
            payload: {
              isAuthenticated: true,
  ...
  const login = async ({username, password}) => {
    ...
    try {
      const response = await axios.post("http://vps65389.dreamhostps.com:5000/api/login", {username, password});
  ...
  const verifySecretKey = async (secretKey) => {
    try {
      const response = await axios.post("http://vps65389.dreamhostps.com:5000/api/verify-secret", { secretKey });
