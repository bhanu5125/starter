...
export function AuthProvider({ children }) {
    ...
    const initialize = async () => {
      try {
        ...
        if (authToken && isTokenValid(authToken)) {
          ...
          const response = await axios.get("https://tcs.trafficcounting.in/nodejs/user/profile");
          dispatch({
            type: "INITIALIZE",
            payload: {
              isAuthenticated: true,
  ...
  const login = async ({username, password}) => {
    ...
    try {
      const response = await axios.post("https://tcs.trafficcounting.in/nodejs/api/login", {username, password});
  ...
  const verifySecretKey = async (secretKey) => {
    try {
      const response = await axios.post("https://tcs.trafficcounting.in/nodejs/api/verify-secret", { secretKey });
