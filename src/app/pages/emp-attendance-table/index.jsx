    ...
    const fetchData = async () => {
      try {
        ...
        const data = response.data.map((staff) => ({
          ...
          attendance: staff.ABSENT || {}, // Default to empty object if not provided
          year: staff.Year, // Map the year directly
          ot: staff.OT || {}, // Default to empty object if not provided
          bonus: staff.BONUS || 0, // Default to empty object if not provided
        }));
        console.log(data);
  
        // Automatically sort employees based on Active status
        const sortedData = [...data].sort((a, b) => {
          return a.status.localeCompare(b.status); // "Active" first, "Inactive" last
        });
