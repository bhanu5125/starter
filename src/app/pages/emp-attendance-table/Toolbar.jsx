...
export function Toolbar({ table, employees = [], setEmployees = () => {}, originalEmployees = [] }) {
  ...
  const handleGenerateClick = () => {
    const filteredEmployees = originalEmployees.filter((employee) => {
      ...
      // Filter by month (check if the selected month has attendance or OT data)
      const monthMatch = !selectedMonth || 
        (employee.attendance[selectedMonth] !== undefined || 
         employee.ot[selectedMonth] !== undefined);

      return departmentMatch && yearMatch && monthMatch;
    });

    // Add selectedMonth to each employee object
    const employeesWithSelectedMonth = filteredEmployees.map(emp => ({
      ...emp,
      selectedMonth, // Pass selectedMonth to each row
    }));

    console.log("Filtered Employees:", employeesWithSelectedMonth);
    setEmployees(employeesWithSelectedMonth);
  };
    ...
    <div className="table-toolbar">
      <div className={clsx("transition-content flex items-center justify-between gap-5", isFullScreenEnabled ? "px-5 sm:px-6" : "px-[--margin-x] pt-4")}>
        <div className="min-w-0">
          <h2 className="truncate px-2 text-xl font-medium tracking-wide text-gray-800 dark:text-dark-50">Attendance List</h2>
        </div>
      </div>
      {isXs ? (
        <>
